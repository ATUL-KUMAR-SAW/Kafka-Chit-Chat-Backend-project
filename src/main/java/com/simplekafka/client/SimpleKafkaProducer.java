...
public class SimpleKafkaProducer {
    ...
    private static final int DEFAULT_PARTITIONS = System.getenv("PARTITION_COUNT") != null 
            ? Integer.parseInt(System.getenv("PARTITION_COUNT")) : 1;
    private static final short DEFAULT_REPLICATION = System.getenv("REPLICATION_FACTOR") != null 
            ? Short.parseShort(System.getenv("REPLICATION_FACTOR")) : 1;
    ...
    public long send(String message) throws IOException {
        return send(message, (String) null);
    }
    
    /**
     * Send a message partitioned by a specific semantic key using consistent hashing
     */
    public long send(String message, String key) throws IOException {
        ...
        int partition = (key == null) ? random.nextInt(partitionCount) 
